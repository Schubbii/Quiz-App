...
function parseQuestionForm(values) {
  const answers = values.options.split(';').map((option) => option.trim()).filter(Boolean);
  const correctAnswerIndex = Number.parseInt(values.correctOption, 10) - 1;

  if (!values.questionText.trim()) throw new Error('Bitte einen Fragetext eingeben.');
  if (!values.category.trim()) throw new Error('Bitte eine Kategorie eingeben.');
  if (answers.length < 2) throw new Error('Bitte mindestens zwei Antwortmöglichkeiten eingeben.');
  if (Number.isNaN(correctAnswerIndex) || correctAnswerIndex < 0 || correctAnswerIndex >= answers.length) {
...
function getAnswerPreview(question) {
  return question.answers.map((answer, index) => `${index + 1}. ${answer}${index === question.correctAnswerIndex ? ' ✓' : ''}`).join(' | ');
}
...
function renderQuestions() {
  ...
  if (questions.length === 0) {
    questionsTableBody.innerHTML = `<tr><td colspan="5" class="empty-state">Noch keine Fragen vorhanden.</td></tr>`;
    return;
  }
...
editModal.addEventListener('click', (event) => {
  if (event.target.dataset.closeModal === 'true') closeEditModal();
});
